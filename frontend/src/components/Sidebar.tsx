...
const sizeClasses: Record<WidgetSize, { grid: string; rounded: string; height: string }> = {
  small:  { grid: 'col-span-1', height: 'h-[144px]', rounded: 'rounded-3xl' },
  medium: { grid: 'col-span-2', height: 'h-[144px]', rounded: 'rounded-2xl' },
  large:  { grid: 'col-span-2', height: 'h-[220px]', rounded: 'rounded-2xl' },
}
...
function EmptyDropZone() {
  ...
  return (
    <div
      ref={setNodeRef}
      className="col-span-2 h-[144px] rounded-2xl bg-[#F2F2F2]/50 flex items-center justify-center"
    >
      <p className="text-sm text-black/30">Drag a widget here</p>
    </div>
  )
}
...
export const Sidebar = forwardRef<SidebarHandle, SidebarProps>(function Sidebar(
  ...
  return (
    ...
    >
      <div
        className="w-[320px] flex-shrink-0 flex flex-col rounded-[20px] p-2 gap-2"
        style={{ height: '100%', background: '#E9E5E6', boxShadow: 'var(--shadow-card)' }}
      >
